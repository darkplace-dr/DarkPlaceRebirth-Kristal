...
function ShadowguyTommyGun:update()
	...
	elseif Game.battle.arena then
		...
		if self.guntimer < 14 then
			for _, enemy in ipairs(self.enemies) do
				enemy.x = MathUtils.lerp(enemy.x, Game.battle.arena.right + 122 + 58, 0.16*DTMULT)
			end
		else
			self.guntimer = 12
			self.firing = true
		...
		self.timer:script(function(wait)
			...
			while Game.battle.wave_timer < Game.battle.wave_length - 29/30 do
				...
				for i = 1, maxbullets do
					...
					local bullet_speed = 5.5 + MathUtils.random(2)
					if self.type == 1 then
						bullet_speed = 4 + MathUtils.random(2)
					end
					...
					while self.rep > 0 do
						local xx, yy = enemy:getRelativePos(0, 0)
						local x, y = xx + 6 + (math.cos(actor.gun_rot) * actor.gunshake), yy  + 63 + (math.sin(actor.gun_rot) * actor.gunshake)
						actor.gun_rot = MathUtils.angle(x, y, Game.battle.arena.x, Game.battle.arena.y) - math.rad(30) + math.rad(MathUtils.randomInt(60))
						if self.type == 1 then
							actor.gun_rot = math.rad(120 - MathUtils.randomInt(90))
						end
						
						self.count = self.count + 1
						if self.count == 5 and #Game.battle.waves < 3 then
							self.count = -10
							actor.gun_rot = MathUtils.angle(x, y, Game.battle.soul.x, Game.battle.soul.y) + math.rad(6) - math.rad(MathUtils.randomInt(12))
						end
