local DogBlack, super = Class(Bullet)

function DogBlack:init(x, y)
    super.init(self, x, y, "battle/bullets/dog_form")
...
function DogBlack:update()
    ...
    if self.collidable == true then
        ...
        if self.activetimer >= 5 and self.times < self.difficulty then
            local bul = self.wave:spawnBullet("rudinn/dog_white", self.x, self.y, self.rotation)
            bul.tp = self.tp
