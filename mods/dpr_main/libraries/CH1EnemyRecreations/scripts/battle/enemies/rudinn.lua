...
function Rudinn:init()
    ...
    if Kristal.getLibConfig("CH1EnemyRecreations", "dog_bullets") then
        table.insert(self.waves,"rudinn/dogs_aim")
    else
        table.insert(self.waves,"rudinn/diamonds_aim")
    end
...
function Rudinn:onAct(battler, name)
    local extra_dialogue = Kristal.getLibConfig("CH1EnemyRecreations", "extra_convince_dialogue")

    if name == "Convince" then
        if extra_dialogue then
            if Game:getFlag("rudinn_convince_tries", 0) < 1 then
                Game.battle:startActCutscene(function(cutscene)
                    ...
                    Game:setFlag("rudinn_convince_tries", Game:getFlag("rudinn_convince_tries", 0) + 1)
                end)
            else
                self:addMercy(100)
    ...
    elseif name == "Lecture" then
        if Kristal.getLibConfig("CH1EnemyRecreations", "tired_animation") then
            self:setAnimation("tired")
        end
        self:setTired(true)
