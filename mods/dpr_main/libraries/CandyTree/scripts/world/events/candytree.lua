...
function CandyTree:init(x, y, properties)
    ...
    self.sprite = Sprite("world/events/candy_tree_3")
    self:addChild(self.sprite)
    self.candies_sprite = Sprite("world/events/candy_tree")
    self.sprite:addChild(self.candies_sprite)
	self.siner = 0
end
...
function CandyTree:onLoad()
    ...
    if self:getFlag("candies", 0) == 1 then
        self.candies_sprite:setFrame(2)
    elseif self:getFlag("candies", 0) == 0 then
        self.candies_sprite:setFrame(3)
	end
end

function CandyTree:update()
    super.update(self)
	if self.candies_sprite then
		self.siner = self.siner + DTMULT
		self.candies_sprite.alpha = 0.7 + (math.sin(self.siner / 8) * 0.3)
	end
end
    ...
    if self:getFlag("candies", 0) == 0 then
        ...
        Game.world:startCutscene(function(cutscene)
            ...
            if choice_i == 1 then
                ...
                if not success then
                    cutscene:text("* (You can't carry any more.)")
                else
                    if self:getFlag("candies", 0) == 2 then
                ...
                cutscene:text("* (You decided to let the tree keep it.)")
			end
        end)
    end
    return true
