...
float outCirc(float time, float start, float change, float duration)
{
    ...
    return change * sqrt(1.0 - pow(time, 2.0)) + start;
}

float outSine(float time, float start, float change, float duration)
{
    return change * sin(time / duration * (3.14 / 2.0)) + start;
}
...
vec4 effect( vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float squishy_percentage = min((1.0 + squish_softening) - (center_dist_x * 2.0), 1.0);
    float total_in_squish = 0.1 * squishiness;

    float dist_y_squish_factor = abs( center_disp_y ) * 2.0;

    float current_in_squish = outCirc( squishy_percentage, 0.0, total_in_squish * sign( center_disp_y ) * outSine( dist_y_squish_factor, 0.0, 1.0, 1.0 ), 1.0 );

    warped_coords.y = warped_coords.y - current_in_squish;

    while (warped_coords.y > 1.0) {
        warped_coords.y -= 1.0;
    }

    while (warped_coords.y < 0.0) {
        warped_coords.y += 1.0;
    }
