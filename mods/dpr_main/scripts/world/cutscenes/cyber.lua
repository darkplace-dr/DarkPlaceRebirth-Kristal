...
local cyber = {
	...
	shoes = function(cutscene, event)
		...
		if Game:hasPartyMember("dess") then
			cutscene:text("* buddy we're [color:red]broke[color:reset] stfu", "mspaint", "dess")
			cutscene:text("* also why are you lowkey blue", "heckyeah", "dess")
			cutscene:text("* you look stupid", "swag", "dess")
			cutscene:text("* ...", nil, event)
			cutscene:text("* That's uhh...\n* That's racist.[wait:5]\n* Go away, please.", nil, event)
            if Game:getFlag("can_kill") == true then
				cutscene:text("[noskip]* no[wait:5]", "teehee", "dess", {auto = true})
				event:explode()
				cutscene:wait(3.5)
