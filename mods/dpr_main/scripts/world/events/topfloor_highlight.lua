...
function TopFloorHighlightArea:init(data)
    ...
    self.middle_x = properties["middle_x"] or data.center_x
		...
		if not chara.no_shadow then
			...
			if hfx then
				if chara.y >= self.y and chara.y <= self.y + self.height then
					if chara.x >= self.x and chara.x <= self.x + (self.width/2) then
						hfx.alpha = MathUtils.lerp(0, 1, (chara.x-self.x)/(self.width/2))
					elseif chara.x >= self.x + (self.width/2) and chara.x <= self.x + (self.width) then
						hfx.alpha = MathUtils.lerp(1, 0, (chara.x-(self.x+self.width/2))/(self.width/2))
					end
				end
			else
