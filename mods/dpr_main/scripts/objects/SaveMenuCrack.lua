---@class SaveMenuCrack : Object
---@overload fun(...) : SaveMenuCrack
local SaveMenuCrack, super = Class(Object)

function SaveMenuCrack:init(save_id, marker)
...
function SaveMenuCrack:updateSaveBoxSize()
...
function SaveMenuCrack:draw()
    ...
    if self.state == "MAIN" then
        ...
        elseif self.selected_x == 2 and self.selected_y == 2 then
            love.graphics.setColor(1, 1, 0)
			if not Game:getConfig("enableRecruits") or #Game:getRecruits(true) <= 0 then
                Draw.setColor(PALETTE["world_gray"])
            end
            love.graphics.print("Recruits", self.box.x + 210, 260)
            love.graphics.setColor(1, 1, 1)
...
function SaveMenuCrack:drawSaveFile(index, data, x, y, selected, header)
