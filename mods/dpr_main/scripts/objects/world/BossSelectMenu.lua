...
function BossSelectMenu:draw()
    ...
    love.graphics.printf("BOSSES", 0, -17, self.width, "center")
    ...
    love.graphics.printf("Page "..self.page_index.."/"..#self.pages, -16, (43 + 40 * (self.bosses_per_page - 1)) + 60, 276, "center")
    love.graphics.setColor(1, 1, 1)
    love.graphics.setFont(self.font)
    ...
    love.graphics.rectangle("line", 300 - info_area_sep_padding, 20, 1, 356)
    love.graphics.setColor(1, 1, 1)
    ...
    love.graphics.draw(preview, 410, 121, 0, 1, 1, preview:getWidth()/2, preview:getHeight()/2)
    ...
    if not boss.locked then
        ...
        love.graphics.printf(description, 280, 202, description_w, "left", 0, description_scale, description_scale)
    else
        local description = string.format(
            "%s",
    ...
    if not boss.locked then
        ...
        love.graphics.printf(stats_1, 280, 262, description_w, "left", 0, 0.5, 1)
        love.graphics.printf(stats_2, 400, 262, description_w, "left", 0, 0.5, 1)
    else
        local stats_1 = string.format(
            "DIFFICULTY\nATTACK     ???\nDEFENSE   ???"
        )
    ...
    for i = 1, 5 do
        ...
        if not boss.locked and boss.difficulty[i] then
            love.graphics.setColor(1, 1, 1)
            Draw.draw(star[1], star_x, 278, 0, 2, 2, 5, 4)
        else
            love.graphics.setColor(0.4, 0.4, 0.4)
            Draw.draw(star[2], star_x, 278, 0, 2, 2, 5, 4)
        end
    end
...
function BossSelectMenu:update()
    ...
    if not OVERLAY_OPEN then
        ...
        local page = self.pages[self.page_index]
        -- move up
        if Input.pressed("up", true) and not Game.battle then
            Assets.playSound("ui_move")
        ...
        -- move down
        if Input.pressed("down", true) and not Game.battle then
            Assets.playSound("ui_move")
            self.selected_index[self.page_index] = warpIndex(self.selected_index[self.page_index] + 1)
            while not page[self.selected_index[self.page_index]] do
                self.selected_index[self.page_index] = warpIndex(self.selected_index[self.page_index] + 1)
