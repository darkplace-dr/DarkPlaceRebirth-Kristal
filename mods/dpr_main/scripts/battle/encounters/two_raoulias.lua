local two_raoulias, super = Class(Encounter)

function two_raoulias:init()
    super.init(self)

    self.text = "* Raoulias flow in!"

    self.music = "battle_inf"
    self.background = true

    self:addEnemy("raoulias")

    self:addEnemy("raoulias")

end

function two_raoulias:onBattleInit()
end

function two_raoulias:onReturnToWorld(events)
    -- check whether the enemies were killed
    if Game.battle.killed then
        -- run this code for each event in the table
        for _,event in ipairs(events) do
            for _,event in ipairs(event:getGroupedEnemies(true)) do
                -- set a 'dont_load' flag to true for the event,
                -- which is a special flag that will prevent the event from loading
                event:setFlag("dont_load", true)
            end
        end
    end
end

return two_raoulias