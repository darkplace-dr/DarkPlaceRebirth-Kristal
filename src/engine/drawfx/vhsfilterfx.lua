local VHSFilterFX, super = Class(FXBase)

function VHSFilterFX:init(priority)
    ...
    self.filter = Assets.getFrames("effects/overlays/vhsfilter")
    self.filter_frame = 1

    self.vhslines = Assets.getTexture("effects/overlays/vhslines")
    self.vhslines_x = -350
...
function VHSFilterFX:draw(texture, object)
...
--[[function VHSFilterFX:getObjectBounds(shader)
