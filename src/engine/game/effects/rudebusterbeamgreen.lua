---@class RudeBusterBeamGreen : Sprite
---@overload fun(...) : RudeBusterBeamGreen
local RudeBusterBeamGreen, super = Class(Sprite)

function RudeBusterBeamGreen:init(x, y, tx, ty, after)
...
function RudeBusterBeamGreen:update()
    ...
    if Utils.dist(self.x, self.y, self.target_x, self.target_y) <= 40 then
        ...
        for i = 1, 8 do
            local burst = RudeBusterBurstGreen(self.target_x, self.target_y, math.rad(45 + ((i - 1) * 90)), i > 4)
    ...
    if self.afterimg_timer >= 1 then
        ...
        local sprite = Sprite("effects/rudebuster/beam_green", self.x, self.y)
