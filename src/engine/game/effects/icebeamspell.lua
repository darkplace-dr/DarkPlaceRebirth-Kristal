---@class IceBeamSpell : Sprite
---@overload fun(...) : IceBeamSpell
local IceBeamSpell, super = Class(Sprite)

function IceBeamSpell:init(red, x, y, tx, ty, after)
...
function IceBeamSpell:update()
    ...
    if Utils.dist(self.x, self.y, self.target_x, self.target_y) <= 40 then
        ...
        for i = 1, 8 do
            local burst = IceBurst(self.red, self.target_x, self.target_y, math.rad(45 + ((i - 1) * 90)), i > 4)
    ...
    if self.afterimg_timer >= 1 then
        ...
        local sprite = Sprite(self.red and "effects/icebeam/arrow_s" or "effects/icebeam/arrow_s", self.x, self.y)
