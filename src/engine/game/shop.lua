...
function Shop:drawBpBonusInfo(current_item, left, top, width)
...
function Shop:drawItemDisplay()
    ...
    elseif current_item.item.type == "badge" then
        self:drawBpBonusInfo(current_item, left, top, width)
    end
...
function Shop:drawStorageDisplay()
    ...
    elseif item_type == "badge" then
        Draw.draw(self.ui_badge_sprite, display_x + 10, 398)
        Draw.draw(self.ui_hold_sprite, display_x + 10, 410)
        love.graphics.print(string.format("%02d", space_count) .. "/" .. string.format("%02d", total_space), display_x + 11, 424, 0, 0.5, 0.5)
        Draw.draw(self.ui_bp_sprite, display_x + 10, 444)
        if current_item.item:getBadgePoints() > (Game.total_bp - Game:getUsedBadgePoints()) then
            Draw.setColor(COLORS.gray)
        end
        love.graphics.print(string.format("%02d", Game:getUsedBadgePoints()) .. "/" .. string.format("%02d", Game.total_bp), display_x + 31, 444, 0, 0.5, 0.5)
        Draw.setColor(COLORS.white)
    else
...
function Shop:addFlag(flag, amount)
    self:setFlag(flag, self:getFlag(flag, 0) + (amount or 1))
    return self:getFlag(flag)
end
